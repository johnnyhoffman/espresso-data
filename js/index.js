...
let fullData;
let clickedDataPoint = null;
let hoveredDataPoint = null;
let clickedOriginalIndexes = [];
let hoveredOriginalIndex = null;
...
const render = (data) => {
    ...
    const lines = linesG.selectAll('.line').data(data.columnDefinitions
    ...
    lines.exit().remove();
    const blobRows = blobsG.selectAll('.blob-row').data(data.columnDefinitions
        .map((o, i) => ({ columnDefinition: o, values: data.values.map(o => o[i]) }))
...
const clearControl = (type, definitionOriginalIndex) => controls[type] = controls[type].filter((o) => o.definitionOriginalIndex != definitionOriginalIndex);
const addControl = (type, control) => {
    clearControl(type, control.definitionOriginalIndex);
    controls[type].push(control);
};
const addRangeControl = (control) => addControl('range', control);
const addDisableControl = (control) => addControl('disable', control);
const addDiscreteControl = (control) => addControl('discrete', control);
const clearRangeControl = (definitionOriginalIndex) => clearControl('range', definitionOriginalIndex);
const clearDisableControl = (definitionOriginalIndex) => clearControl('disable', definitionOriginalIndex);
const clearDiscreteControl = (definitionOriginalIndex) => clearControl('discrete', definitionOriginalIndex);
const getCurrentIndexFromOriginal = (columnDefinitions, originalIndex) => {
...
const sortDefinitions = (definitions) => definitions
    .map((d, i) => ({ i, d }))
    .sort((a, b) => {
